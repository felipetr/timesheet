...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$BMP = unpack('Vheader_size/Vwidth/Vheight/vplanes/vbiper_pixel' .
			'/Vcompression/Vsize_bitmap/Vhoriz_resolution' .
			'/Vvert_resolution/Vcolors_used/Vcolors_important', fread($f1, 40));
		$BMP['colors'] = pow(2, $BMP['biper_pixel']);
		...
		$BMP['bytes_per_pixel'] = $BMP['biper_pixel'] / 8;
		...
		while ($Y >= 0)
		{
			...
			while ($X < $BMP['width'])
			{
				if ($BMP['biper_pixel'] == 24)
				{
					$COLOR = unpack("V", substr($IMG, $P, 3) . $VIDE);
				}
				elseif ($BMP['biper_pixel'] == 16)
				{
           ...
           *
           * Notes:
           * "don't work with bmp 16 biper_pixel. change pixel
           * generator for this."
           *
           */
				...
				elseif ($BMP['biper_pixel'] == 8)
				{
					$COLOR = unpack("n", $VIDE . substr($IMG, $P, 1));
					$COLOR[1] = $PALETTE[ $COLOR[1] + 1 ];
				}
				elseif ($BMP['biper_pixel'] == 4)
				{
