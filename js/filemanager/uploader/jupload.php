...
class JUpload {
	...
	private function cleanup() {
		...
		// reset session var
		$_SESSION['ts_RF'][$this->classparams['var_prefix'].'size'] = 0;
		return;
	}
...
private function receive_uploaded_files() {
	...
	if (!isset($_SESSION['ts_RF'][$this->classparams['var_prefix'].'size'])) {
		$this->abort('Invalid session (in afterupload, POST, check of size)');
	}
	if (!isset($_SESSION['ts_RF'][$this->classparams['var_prefix'].'files'])) {
		$this->abort('Invalid session (in afterupload, POST, check of files)');
	}
	$this->files = $_SESSION['ts_RF'][$this->classparams['var_prefix'].'files'];
...
if ($jupart) {
	...
	$_SESSION['ts_RF'][$this->classparams['var_prefix'].'size'] += $len;
	if ($len > 0) {
	...
	if ($jufinal) {
		...
		if ($dlen != $_SESSION['ts_RF'][$this->classparams['var_prefix'].'size'])
...
echo $this->appletparams['stringUploadSuccess']."\n";
$_SESSION['ts_RF'][$this->classparams['var_prefix'].'files'] = $this->files;
session_write_close();
exit;
}
...
private function page_start() {
	...
	if ($_SERVER['REQUEST_METHOD'] == 'HEAD') {
		...
		if (isset($_GET['afterupload'])) {
			...
			$_SESSION['ts_RF'][$this->classparams['var_prefix'].'files'] = array();
