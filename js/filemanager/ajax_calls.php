...
if (isset($_SESSION['ts_RF']['language_file']) && file_exists($_SESSION['ts_RF']['language_file']))
{
	include $_SESSION['ts_RF']['language_file'];
} else {
...
if(isset($_GET['action']))
{
	switch($_GET['action'])
	{
		case 'view':
			if(isset($_GET['type']))
			{
				$_SESSION['ts_RF']["view_type"] = $_GET['type'];
			}
			else
			{
		...
		case 'filter':
			if (isset($_GET['type']))
			{
				...
				{
					$_SESSION['ts_RF']["filter"] = $_GET['type'];
				}
			}
			else {
				response(trans('view type number missing').AddErrorLocation())->send();
		...
		case 'sort':
			...
			{
				$_SESSION['ts_RF']["sort_by"] = $_GET['sort_by'];
			}

			if (isset($_GET['descending']))
			{
				$_SESSION['ts_RF']["descending"] = $_GET['descending'];
			}
			break;
		...
		case 'copy_cut':
			...
			$_SESSION['ts_RF']['clipboard']['path'] = $_POST['path'];
			$_SESSION['ts_RF']['clipboard_action'] = $_POST['sub_action'];
			break;
		case 'clear_clipboard':
			$_SESSION['ts_RF']['clipboard'] = null;
			$_SESSION['ts_RF']['clipboard_action'] = null;
			break;
		case 'chmod':
			$path = $current_path . $_POST['path'];
			if (
		...
		case 'change_lang':
			...
			$_SESSION['ts_RF']['language'] = $choosen_lang;
			$_SESSION['ts_RF']['language_file'] = 'lang/' . $choosen_lang . '.php';
			//Tracevar($_SESSION['ts_RF']);
