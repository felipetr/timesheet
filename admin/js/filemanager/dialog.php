...
$_SESSION['ts_adm_RF']["verify"] = "RESPONSIVEfilemanager";
...
if (!isset($_SESSION['ts_adm_RF']["subfolder"]))
{
	$_SESSION['ts_adm_RF']["subfolder"] = '';
}
$rfm_subfolder = '';

if (!empty($_SESSION['ts_adm_RF']["subfolder"]) && strpos($_SESSION['ts_adm_RF']["subfolder"],'../') === FALSE
&& strpos($_SESSION['ts_adm_RF']["subfolder"],'./') === FALSE && strpos($_SESSION['ts_adm_RF']["subfolder"],"/") !== 0
&& strpos($_SESSION['ts_adm_RF']["subfolder"],'.') === FALSE)
{
	$rfm_subfolder = $_SESSION['ts_adm_RF']['subfolder'];
}
...
//view type
if(!isset($_SESSION['ts_adm_RF']["view_type"]))
{
	$view = $default_view;
	$_SESSION['ts_adm_RF']["view_type"] = $view;
}
...
$view = $_SESSION['ts_adm_RF']["view_type"];

//filter
$filter = "";
if(isset($_SESSION['ts_adm_RF']["filter"]))
{
	$filter = $_SESSION['ts_adm_RF']["filter"];
}
...
if (!isset($_SESSION['ts_adm_RF']['sort_by']))
{
	$_SESSION['ts_adm_RF']['sort_by'] = 'name';
}

if (isset($_GET["sort_by"]))
{
	$sort_by = $_SESSION['ts_adm_RF']['sort_by'] = fix_get_params($_GET["sort_by"]);
} else $sort_by = $_SESSION['ts_adm_RF']['sort_by'];


if (!isset($_SESSION['ts_adm_RF']['descending']))
{
	$_SESSION['ts_adm_RF']['descending'] = TRUE;
}

if (isset($_GET["descending"]))
{
	$descending = $_SESSION['ts_adm_RF']['descending'] = fix_get_params($_GET["descending"])==1;
} else {
	$descending = $_SESSION['ts_adm_RF']['descending'];
}
...
if (isset($_GET['lang']))
{
	...
	$_SESSION['ts_adm_RF']['language'] = $lang;
	$_SESSION['ts_adm_RF']['language_file'] = 'lang/' . $lang . '.php';
}
...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<script>
		...
		<?php
			foreach ($aviary_defaulconfig as $aopt_key => $aopt_val) {
...
<body>
	...
	<input type="hidden" id="clipboard" value="<?php echo ((isset($_SESSION['ts_adm_RF']['clipboard']['path']) && trim($_SESSION['ts_adm_RF']['clipboard']['path']) != null) ? 1 : 0);?>" />
